...
complementarity_matrix = pd.read_excel("互补性矩阵.xlsx", index_col=0)
substitution_matrix = pd.read_excel("替代性矩阵.xlsx", index_col=0)
...
for sim in range(n_simulations):
    ...
    # 创建模型
    model = LpProblem(name="crop_optimization", sense=LpMaximize)

    # 定义决策变量
    variables = {}
    for plot in plots:
        for crop in crops_df['作物名称']:
            var_name = f"{plot['地块名称']}_{crop}"
            variables[var_name] = LpVariable(var_name, lowBound=0, cat='Continuous')

    # 目标函数
    objective = lpSum(
        variables[plot['地块名称'] + '_' + crop['作物名称']] *
        (crop['销售单价/(元/斤)'] * crop['亩产量/斤'] - crop['种植成本/(元/亩)'])
        for plot in plots
    ...
    for plot1 in plots:
        for plot2 in plots:
            if plot1 != plot2:
                for crop1 in crops_df['作物名称']:
                    for crop2 in crops_df['作物名称']:
                        if crop1 != crop2:
                            objective += complementarity_matrix.loc[crop1, crop2] * variables[plot1['地块名称'] + '_' + crop1] * variables[plot2['地块名称'] + '_' + crop2]
                            objective -= substitution_matrix.loc[crop1, crop2] * variables[plot1['地块名称'] + '_' + crop1] * variables[plot2['地块名称'] + '_' + crop2]

    # 添加约束条件
    for plot in plots:
        model += lpSum(variables[plot['地块名称'] + '_' + crop] for crop in crops_df['作物名称']) <= plot['地块面积/亩']
